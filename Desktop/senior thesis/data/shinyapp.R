...
server <- function(input, output, session) {
  ...
  output$tableVar1 <- renderTable({
    ...
    # Create a table of counts based on the selected variable
    if (selected_variable == "Condition") {
      table_data <- table(dataInput()$Condition)
      table_df <- as.data.frame(table_data)
      colnames(table_df) <- c("Condition", "Count")
      return(table_df)
    } else {
      # Display means for other variables
