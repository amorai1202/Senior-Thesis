...
centrality_mod <- aov(Score ~ Condition*Centrality_F, data = centrality_only)
emmeans(centrality_mod, specs = c("Condition", "Centrality_F"))
#Subtle/Blatant High vs Low - test contrasts

ggplot(data = highC, aes(x = Condition, y = Score, fill = Centrality_F)) +
  geom_boxplot() +
  theme_bw()
